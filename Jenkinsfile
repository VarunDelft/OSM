pipeline {
  ...
  stages {
    ...
    stage('Test') {
      steps {
        sh "sshpass -p ${props.CLIENT_PWD} ssh  -o 'StrictHostKeyChecking=no' ${props.CLIENT_UID}@${props.CLIENT_IP} TestScripts/TestServerConnectivity.sh ${props.FIREWALL_IP}"        
      }
    }
    ...
    post {
        ...
        success {
            echo 'I succeeeded!'
            git mv FirewallConfig FirewallConfig_b1
            sh "git commit -m 'abc'"
